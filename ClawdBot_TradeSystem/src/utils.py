...
def analyze_with_gemini(image_path):
    ...
    try:
        ...
        # 使用 7a4ce1f 的提示词格式
        prompt = """你是一个严格执行规则的缠论多周期分析师，分析必须按以下步骤进行：
...
**关键规则**：
- ✅ **只关注图表右侧区域**（K线最右端的可见区域）
- ✅ **位置对应**：左侧=1分钟，右上=5分钟，右下=25分钟
- ✅ **如果有多个标注在同一区域，全部识别并列出**
- ❌ **忽略左侧的历史标注**

**输出格式**：
- 1分钟可见标注：[右侧可见区域标注，用"+"连接]
- 5分钟可见标注：[右侧可见区域标注，用"+"连接]
- 25分钟可见标注：[右侧可见区域标注，用"+"连接]
...
**首先统计各周期卖点/买点**：
- 卖点优先级：3卖 > 2卖 > 1卖（非预期优先）
- 买点优先级：3买 > 2买 > 1买（非预期优先）
